...
export default function VideoListClient({ initialVideos }: { initialVideos: AdminVideo[] }) {
  ...
  const deleteMutation = useMutation({
    ...
    onMutate: async (id: string) => {
      ...
      const previous = queryClient.getQueryData<AdminVideo[]>(["admin-videos"]);
      queryClient.setQueryData<AdminVideo[]>(["admin-videos"], (old) =>
    ...
    onError: (_err, _id, context) => {
      if (context?.previous) {
        queryClient.setQueryData(["admin-videos"], context.previous);
      }
    },
    onSettled: () => {
  ...
  return (
    <div>
      {/* Header — lives here so count stays in sync with query data */}
      ...
      <div className="bg-charcoal border border-smoke/40 rounded-sm overflow-hidden">
        {videos.map((v: AdminVideo, i: number) => {
          ...
          return (
            <div
              key={v.id}
              className={`flex items-center gap-3 sm:gap-4 px-4 sm:px-5 py-3 transition-opacity ${
